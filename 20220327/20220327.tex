...
Last time we showed a weaker version of a lemma from Waldhausen.

\begin{lem} \label{lem:waldhausen}
	Let \(M\) be an irreducible compact manifold. Let \(S, S'\) be two incompressible compact, closed boundary components of \(M\). Suppose: if \(k\) is any closed curves in \(S\), then some non-null multiple of \(k\) is homotopic to a curve in \(S'\). Then \(M\) is homeomorphic to \(S\times I\).
\end{lem}

Today we are going to use Lemma \ref{lem:waldhausen} to show the following theorem.

\begin{sat}
	Let \(M\) be a \(3\)-manifold, and let \(S\) be a surface. Suppose \(\pi_1(M)\cong\pi_1(S)\). Then \(M \cong S \times \)
\end{sat}
